...
class JanelaHashes(QWidget):
    ...
    def mostrar_sobre(self):
        ...
        conteudo_html = (
            ...
            "<li><b>Gatilhos de Alerta Forense:</b> Se a extensão autêntica for incompatível com a extensão disfarçada no nome do arquivo (ex: um executável ou script malicioso camuflado de .pdf ou imagem), o extrator injeta automaticamente no laudo a tag <b>\"⚠️ ALERTA FORENSE: POSSÍVEL ADULTERAÇÃO DE EXTENSÃO DETECTADA (Magic Bytes) ⚠️\"</b>, expondo a verdadeira natureza estrutural do arquivo. <i>(Nota: O sistema possui uma whitelist inteligente para extensões atípicas baseadas em texto plano, aplicando um aviso brando para evitar falsos positivos na triagem).</i></li>"
            "</ul>"
            ...
            "<li><b>Compatibilidade do Sistema Operacional (Limitação Crítica):</b> As funcionalidades gerais de extração de hashes e metadados operam normalmente em versões mais antigas do Windows (como o Windows 7 e 8.1), desde que suportadas pelo ambiente gráfico. Contudo, as ferramentas avançadas de <b>Aquisição de Imagem Forense (.E01)</b> e de ativação do <b>Software Write-Blocker</b> dependem de recursos modernos, APIs e diretivas de execução exclusivas do <i>PowerShell 5.1</i> (como codificação BOM segura, encapsulamento de arrays e redirecionamentos lógicos). Por conta disso, <b>estas funções específicas exigem rigorosamente o Windows 10 ou superior</b>, sob risco de falha (código de erro 4294770688) em versões anteriores do sistema.</li>"
            "</ul>"
        )
        ...
        conteudo_agradecimentos_html = (
            ...
            "<i>Phil Harvey (ExifTool)</i>, a equipe do <i>MediaInfo</i>, os mantenedores do projeto <i>file (libmagic)</i>, "
            "e os desenvolvedores do <i>PySide6/Qt</i>, <i>Pillow</i>, <i>pypdf</i>, <i>LnkParse3</i>, <i>pefile</i> e <i>python-magic</i>. "
            "Um agradecimento adicional à comunidade da <i>Alpine Security</i> por disponibilizar os binários compilados do ewf-tools para Windows, "
            "e ao projeto <i>MSYS2 / MinGW-w64</i> pelo fornecimento do ambiente de bibliotecas nativas essenciais para a análise profunda.</li>"
            "</ul>"
